...
class CompanyBloc extends Bloc<CompanyEvent, CompanyState> {
  ...
  Future<void> _onLoadCompanies(LoadCompanies event, Emitter<CompanyState> emit) async {
    ...
    try {
      ...
      if (companies.isEmpty) {
        emit(state.copyWith(companies: [], selectedCompany: null, members: [], invitations: [], isLoading: false));
        return;
      }

      // Keep current selection if still valid, otherwise use the one saved in local storage
      final currentSelectedId = state.selectedCompany?['id']?.toString();
      Map<String, dynamic>? targetCompany;

      if (currentSelectedId != null) {
        for (final c in companies) {
          if (c['id']?.toString() == currentSelectedId) { targetCompany = c; break; }
        }
      }

      // Only default to first company if user hasn't explicitly selected one
      ...
      if (companyId != null && companyId.isNotEmpty) {
        try { members = await _authService.getCompanyMembers(companyId); } catch (e) { debugPrint('Failed to load members: $e'); }
        try { invitations = await _authService.getPendingInvitations(companyId); } catch (e) { debugPrint('Failed to load invitations: $e'); }
        ...
        try {
          ...
          // Don't emit error - this is non-critical, inventory will use fallback
        }
      }
      ...
      emit(state.copyWith(isLoading: false, error: 'Failed to load companies: $e'));
    }
  }

  Future<void> _onSelectCompany(SelectCompany event, Emitter<CompanyState> emit) async {
    if (state.selectedCompany?['id']?.toString() == event.companyId && state.members.isNotEmpty) return;
    ...
    try {
      ...
      // Update InventoryService context
      try {
      ...
      List<Map<String, dynamic>> members = [];
      List<Map<String, dynamic>> invitations = [];
      try { members = await _authService.getCompanyMembers(event.companyId); } catch (e) { debugPrint('Failed to load members: $e'); }
      try { invitations = await _authService.getPendingInvitations(event.companyId); } catch (e) { debugPrint('Failed to load invitations: $e'); }

      emit(state.copyWith(selectedCompany: targetCompany, members: members, invitations: invitations, isLoading: false));
    } catch (e) {
      emit(state.copyWith(isLoading: false, error: 'Failed to select company: $e'));
    }
  }

  Future<void> _onCreateCompany(CreateCompany event, Emitter<CompanyState> emit) async {
    emit(state.copyWith(isLoading: true, error: null));
    try {
  ...
  Future<void> _onDeleteCompany(DeleteCompany event, Emitter<CompanyState> emit) async {
    final company = _findCompany(event.companyId);
    final role = company?['role']?.toString() ?? 'viewer';
    if (role != 'owner') { emit(state.copyWith(error: 'Only owners can delete a company')); return; }
  ...
  Future<void> _onLeaveCompany(LeaveCompany event, Emitter<CompanyState> emit) async {
    emit(state.copyWith(isLoading: true, error: null));
    try {
  ...
  Future<void> _onJoinCompany(JoinCompany event, Emitter<CompanyState> emit) async {
    emit(state.copyWith(isLoading: true, error: null));
    try {
      final result = await _authService.acceptInvitation(event.token);
      if (result == null) { emit(state.copyWith(isLoading: false, error: 'Invalid or expired invitation token')); return; }
      final success = result['success'];
      if (success == true) {
        emit(state.copyWith(successMessage: 'Company joined successfully!', isLoading: false));
        add(const LoadCompanies());
      } else {
        emit(state.copyWith(isLoading: false, error: result['message']?.toString() ?? 'Invalid or expired invitation token'));
      }
    } catch (e) { emit(state.copyWith(isLoading: false, error: 'Error: ${e.toString()}')); }
  }

  Future<void> _onCreateInvitation(CreateInvitation event, Emitter<CompanyState> emit) async {
    emit(state.copyWith(isLoading: true, error: null));
    try {
      final result = await _authService.createInvitation(companyId: event.companyId, email: event.email, role: event.role);
      ...
      if (success == true || success?.toString() == 'true') {
        ...
        emit(state.copyWith(isLoading: false, error: result['message']?.toString() ?? 'Failed to send invitation'));
      }
    } catch (e) { emit(state.copyWith(isLoading: false, error: 'Error: ${e.toString()}')); }
  }

  Future<void> _onCancelInvitation(CancelInvitation event, Emitter<CompanyState> emit) async {
    try {
  ...
  Future<void> _onRemoveMember(RemoveMember event, Emitter<CompanyState> emit) async {
    try {
  ...
  Future<void> _onChangeMemberRole(ChangeMemberRole event, Emitter<CompanyState> emit) async {
    try {
  ...
  Future<void> _onRefreshCompanyData(RefreshCompanyData event, Emitter<CompanyState> emit) async {
    try {
      List<Map<String, dynamic>> members = [];
      List<Map<String, dynamic>> invitations = [];
      try { members = await _authService.getCompanyMembers(event.companyId); } catch (e) { debugPrint('Failed to load members: $e'); }
    ...
    } catch (e) { debugPrint('RefreshCompanyData error: $e'); }
  }
