...
class _CompanySettingsScreenState extends State<CompanySettingsScreen> {
  ...
  bool get _canManageMembers {
    ...
    final role = (company['role'] ?? company['user_role'] ?? company['membership_role']).toString().toLowerCase();
  ...
  void _inviteUser() {
    ...
    if (companyId == null || companyId.isEmpty) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('No company selected.'), backgroundColor: Colors.orange),
      );
      return;
    }
  ...
  void _removeMember(String memberId, String memberName) async {
    ...
    if (confirm == true && mounted) {
      context.read<CompanyBloc>().add(RemoveMember(
            memberId: memberId, companyId: companyId, memberName: memberName,
          ));
    }
  }
  ...
  void _changeRole(String memberId, String currentRole) async {
      ...
      builder: (ctx) => SimpleDialog(
        ...
        children: ['admin', 'data_operator', 'viewer']
            .map((role) => SimpleDialogOption(
                  ...
                  child: Row(children: [
                    if (role == currentRole) const Icon(Icons.check, size: 18),
                    const SizedBox(width: 8),
                    Text(role == 'data_operator' ? 'Data Operator' : role[0].toUpperCase() + role.substring(1)),
                  ]),
                ))
            .toList(),
      ),
    ...
    if (newRole != null && newRole != currentRole && mounted) {
      context.read<CompanyBloc>().add(ChangeMemberRole(
            memberId: memberId, companyId: companyId, newRole: newRole,
          ));
    }
  }
    ...
    return BlocListener<CompanyBloc, CompanyState>(
      ...
      child: BlocBuilder<CompanyBloc, CompanyState>(
        builder: (context, state) {
              ...
              body: Center(
                child: Padding(
                  ...
                  child: Column(mainAxisSize: MainAxisSize.min, children: [
                    ...
                    const Text('No company selected', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600)),
                    const SizedBox(height: 8),
                    const Text('Please go back and select a company first.',
          ...
          return Scaffold(
            appBar: AppBar(
              title: Text(widget.inventoryName.isNotEmpty ? '${widget.inventoryName} - Members' : 'Company Settings'),
            ),
                    ...
                    child: Padding(
                      ...
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Row(children: [
                          ...
                          const SizedBox(width: 8),
                          Text('Company Members (${state.members.length})',
                            ...
                            return ListTile(
                              ...
                              trailing: _canManageMembers && !isOwner
                                  ? PopupMenuButton<String>(
                                      onSelected: (action) {
                                        if (action == 'change_role') _changeRole(memberId, role);
                                        if (action == 'remove') _removeMember(memberId, name);
                                      },
                                      itemBuilder: (ctx) => [
                      ...
                      child: Padding(
                        ...
                        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          ...
                          DropdownButtonFormField<String>(
                            ...
                            items: const [
                              DropdownMenuItem(value: 'admin', child: Text('Admin - Full access except delete company')),
                              DropdownMenuItem(value: 'data_operator',
                                  child: Text('Data Operator - Create/update items, no delete')),
                      ...
                      child: Padding(
                        ...
                        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          Row(children: [
                            ...
                            const SizedBox(width: 8),
                            Text('Pending Invitations (${state.invitations.length})',
                                style: theme.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
                          ]),
                          const SizedBox(height: 12),
