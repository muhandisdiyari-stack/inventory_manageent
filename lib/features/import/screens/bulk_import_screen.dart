...
class _BulkImportScreenState extends State<BulkImportScreen> {
  ...
  void _downloadTemplate() {
    ...
    if (mounted) {
      SnackBarUtils.success(
          context, 'Template ready! Columns: ${enabledFields.join(", ")}');
    }
  }
  ...
  Future<void> _saveLocally(List<int> bytes, String fileName) async {
    try {
      if (Platform.isAndroid) {
        ...
        if (mounted) {
          SnackBarUtils.info(context, 'Saved to app storage: $savedPath');
        }
        ...
        await File(savedPath).writeAsBytes(bytes);
        if (mounted) {
          SnackBarUtils.info(context, 'Saved to: $savedPath');
        }
      ...
      if (mounted) {
        SnackBarUtils.error(context, 'Could not save file: $e');
      }
    }
  }
        ...
        for (final path in possiblePaths) {
          ...
          if (await dir.exists()) {
            try {
              ...
              if (mounted) {
                SnackBarUtils.success(
                    context, 'Saved to Downloads folder: $fileName');
              }
              return true;
            } catch (e) {
              continue;
  ...
  Future<void> _pickFile() async {
    try {
      ...
      if (!mapping.containsKey(_FieldNames.name) &&
          headers.isNotEmpty) {
      ...
      if (mounted) {
        if (unmapped.isEmpty) {
          SnackBarUtils.success(
              context, 'Ready! ${dataRows.length} items found.');
        } else {
  ...
  Future<void> _startImport() async {
    if (_rows == null ||
        !_fieldMapping.containsKey(_FieldNames.name)) {
      if (mounted) {
        SnackBarUtils.error(
            context, 'Name column is required for import');
      }
      return;
    }
    ...
    try {
      ...
      for (var entry in byName.entries) {
        ...
        _updateProgress(0.2 + 0.75 * (processed / total),
      ...
      if (!mounted) return;
      setState(() => _isLoading = false);
      SnackBarUtils.error(context, 'Import failed: $e');
    }
  }
    ...
    return Column(children: [
      Container(
        ...
        child: Row(children: [
          ...
          FilledButton(
              onPressed: unmappedRequired.isEmpty
                  ? _startImport
                  : null,
          ...
          itemBuilder: (_, i) {
            ...
            return Card(
              ...
              child: ListTile(
                ...
                subtitle: Text(
                    '${stats.itemsImported} imported${stats.duplicatesSkipped > 0 ? ', ${stats.duplicatesSkipped} duplicates' : ''}${stats.rowsSkipped > 0 ? ', ${stats.rowsSkipped} errors' : ''}',
                    style: TextStyle(
                        fontSize: 11,
