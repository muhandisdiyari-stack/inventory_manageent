...
class InventoryService {
    ...
    if (_currentCompanyId != companyId) {
      ...
      if (_currentInventoryId != null) {
        try {
          ...
          debugPrint('⚠️ Failed to reinitialize: $e');
        }
      }
    }
  }

  /// Load company ID from inventory_members (new schema - inventory-based access).
  /// Falls back to profiles table if no inventory membership found.
  Future<void> _loadCompanyId() async {
    ...
    try {
      ...
      // Query inventory_members to find company (new schema)
  ...
  void _loadLabelsFromCache(String inventoryId) {
    ...
    _labelsCache[inventoryId] = (raw is List && raw.isNotEmpty)
        ? raw.map((e) => Label.fromLocalJson(Map<String, dynamic>.from(e))).toList()
        : [];
  }
        ...
        try {
          ...
          for (final existingItem in box.values) {
            if (existingItem.supabaseId == item.supabaseId) {
              ...
              existingItem.labelId = item.labelId;
          ...
          debugPrint('⚠️ Error syncing item: $e');
        }
      }
  ...
  InventoryItem _itemFromSupabaseRow(Map<String, dynamic> row) {
    ...
    final labelName = row['labels'] is Map
        ? (row['labels'] as Map)['name']?.toString() ?? ''
        : (row['label'] as String?) ?? '';
    ...
    item.labelId = row['label_id'] as String?;
    item.isSynced = true;
    return item;
  }
  ...
  List<Label> get labels =>
      _currentInventoryId != null ? (_labelsCache[_currentInventoryId!] ?? []) : [];
  ...
  Future<void> deleteLabel(String name) async {
    ...
    if (box != null) {
      final toDelete =
          box.values.where((item) => item.label == name).toList();
  ...
  List<InventoryItem> getItemsByLabel(String label) {
    if (_currentInventoryId == null ||
        !_itemsBoxes.containsKey(_currentInventoryId!)) return [];
  ...
  Future<void> _syncItemToSupabase(InventoryItem item) async {
    ...
    try {
      ...
      try {
        final offlineSync =
            OfflineSyncService(supabaseClient: SupabaseClientService());
        await offlineSync.loadPendingMutations();
        await offlineSync.queueMutation(
          mutationKey:
              'item_${item.id}_${DateTime.now().microsecondsSinceEpoch}_${item.quantity}',
          table: 'inventory_items',
          data: item.toSupabaseJson(),
          operation:
              item.supabaseId != null && item.supabaseId!.isNotEmpty
                  ? 'update'
                  : 'insert',
        );
      } catch (_) {}
  ...
  List<Map<String, dynamic>> searchAllInventories(String query) {
        ...
        for (final item in box.values) {
          if (item.matchesQuery(lowerQuery)) {
            results.add({
              ...
              'inventoryName': inventoryNames[inventoryId] ??
    ...
    results.sort((a, b) =>
        (a['item'] as InventoryItem)
            .name
            .toLowerCase()
            .compareTo((b['item'] as InventoryItem).name.toLowerCase()));
    return results;
  }
  ...
  Map<String, String> getAllInventoryNames() {
    ...
    try {
      ...
      final d = box.get('cached_inventories');
      if (d is List) {
        for (final i in d) {
          if (i is Map) {
            names[i['id']?.toString() ?? ''] =
  ...
  String getInventoryName(String inventoryId) {
    try {
      ...
      if (d is List) {
        for (final i in d) {
          if (i is Map && i['id']?.toString() == inventoryId) {
            return i['name']?.toString() ?? inventoryId;
          }
        }
      }
    } catch (_) {}
  ...
  InventorySettings? get currentSettings {
    if (_currentInventoryId == null ||
        !_settingsBoxes.containsKey(_currentInventoryId!)) return null;
