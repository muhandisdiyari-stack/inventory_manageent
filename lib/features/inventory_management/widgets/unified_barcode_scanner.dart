...
import 'mobile_camera_scanner_sheet.dart';
...
class UnifiedBarcodeScanner {
  ...
  static Future<void> showScanner(
    ...
    try {
      ...
      if (supportedTypes.isEmpty) {
        _showMessage(context, 'Barcode scanning is not supported on this device');
        return;
      }
      ...
      _showMessage(context, 'Failed to open scanner: ${e.toString()}');
    }
  }

  static Future<void> _openScanner(
    BuildContext context,
    ...
    switch (type) {
      case BarcodeScannerType.camera:
        await _openCameraScanner(context, onBarcodeScanned);
        break;

      case BarcodeScannerType.image:
        await ImageBarcodeScanner.pickAndScan(
          context,
  ...
  static Future<void> _openCameraScanner(
    BuildContext context,
    Function(String) onBarcodeScanned,
  ) async {
    try {
      if (defaultTargetPlatform == TargetPlatform.android ||
        ...
        await MobileCameraScannerSheet.show(
          context,
          onBarcodeScanned: onBarcodeScanned,
        );
      }
    } catch (e) {
      debugPrint('Camera scanner error: $e');
      if (context.mounted) {
        _showMessage(context, 'Camera scanner unavailable. Try keyboard or image scanning.');
      }
    }
  }
