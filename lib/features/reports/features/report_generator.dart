...
class ReportGenerator {
  ...
  Future<ReportResult> generateReport({
    ...
    // Generate CSV in an isolate for large datasets
    final csvString = await compute((_) {
      return csvService.generateCsvWithFields(
        filteredItems,
        settings,
        inventoryName,
        reportType == 'all' ? 'All Items' : reportType == 'expiring' ? 'Expiring Soon' : 'Expired',
        selectedFieldsList,
      );
    }, null);
    ...
    final fileName = '${safeName}_report_${ReportUtils.formatTimestamp(timestamp)}.csv';
