...
class _SearchScreenState extends State<SearchScreen> {
  ...
  void _loadPermissions() {
    try {
      ...
      _permissions = user?.inventoryPermissions ?? InventoryPermissions.fromRole(user?.role.name ?? 'viewer');
    } catch (_) {
  ...
  void _performSearch(String query) {
    ...
    if (query.trim().isEmpty) {
      setState(() { _isSearching = false; _hasSearched = false; });
      return;
    }
  ...
  void _editItem(InventoryItem item) async {
    if (!_canUpdate) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('You do not have permission to edit items'), backgroundColor: Colors.red),
      );
      return;
    }
  ...
  void _adjustQuantity(InventoryItem item, int delta) {
    if (!_canUpdate) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('You do not have permission to update quantities'), backgroundColor: Colors.red),
      );
      return;
    }
          ...
          if (_isSearching) {
            return const Center(
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                CircularProgressIndicator(), SizedBox(height: 16), Text('Searching...'),
              ]),
            );
          }
  ...
  Widget _buildInitialState() {
    return Center(
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Text('Search by name, code, barcode, or size', style: TextStyle(color: Colors.grey[500])),
        const SizedBox(height: 24),
        Wrap(
          spacing: 8, runSpacing: 8,
          children: ['name', 'code', 'barcode', 'size', 'color', 'material']
              .map((f) => Chip(label: Text(f, style: const TextStyle(fontSize: 12)), visualDensity: VisualDensity.compact))
              .toList(),
        ),
      ]),
    );
        ...
        child: ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                child: Row(children: [
                  Container(
                    ...
                    child: Row(mainAxisSize: MainAxisSize.min, children: [
                      ...
                      const SizedBox(width: 8),
                      Text(inventoryName,
                          style: TextStyle(fontWeight: FontWeight.w700, color: Theme.of(context).colorScheme.primary, fontSize: 15)),
                    ]),
                  ),
                  const Spacer(),
                  TextButton.icon(
                    onPressed: () {
                      ...
                      if (inventoryId != null) _navigateToInventory(inventoryId);
                    },
              ...
              ...items.map((result) => _buildItemCard(result['item'] as InventoryItem, inventoryName)),
              const SizedBox(height: 8),
            ]);
          },
        ),
      ...
      child: ExpansionTile(
        leading: CircleAvatar(
          ...
          child: Icon(Icons.inventory_2, size: 20,
              color: item.isExpired ? Colors.red : item.isExpiringSoon ? Colors.orange : Theme.of(context).colorScheme.primary),
        ),
        title: Row(children: [
          Expanded(
          ...
          if (item.isExpired) _buildBadge('EXPIRED', Colors.red),
          if (item.isExpiringSoon) _buildBadge('EXPIRING', Colors.orange),
          if (item.hasReachedUpdateLimit) _buildBadge('LOCKED', Colors.purple),
        ]),
        subtitle: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text('👤 ${item.creatorDisplayName}', style: TextStyle(fontSize: 10, color: Colors.grey[500])),
          if (item.updateCount > 0)
            Text('🔄 ${item.updateCount}/3 updates', style: TextStyle(fontSize: 10, color: Colors.grey[400])),
        ]),
        trailing: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          Text('${item.quantity}', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w700)),
        ...
        children: [
          Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              if (item.productionDate != null) _infoRow('Production', item.productionDate.toString().split(' ')[0]),
              if (item.expireDate != null) _infoRow('Expires', item.expireDate.toString().split(' ')[0]),
              ...
              const SizedBox(height: 8),
              if (_canUpdate && !item.hasReachedUpdateLimit)
                Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
                  ...
                  IconButton.filled(
                    onPressed: item.quantity < InventoryItem.maxQuantity ? () => _adjustQuantity(item, 1) : null,
                  ...
                  IconButton(onPressed: () => _editItem(item), icon: const Icon(Icons.edit, size: 20), tooltip: 'Edit item'),
                ])
                ...
                Container(
                  ...
                  child: const Row(children: [
                    Icon(Icons.lock, size: 14, color: Colors.purple),
                    SizedBox(width: 8),
                    Flexible(child: Text('Update limit reached', style: TextStyle(fontSize: 11, color: Colors.purple))),
                  ]),
                )
              else
                Center(
                  child: Text('View only', style: TextStyle(fontSize: 11, color: Colors.grey[500])),
                ),
            ]),
          ),
        ],
  ...
  Widget _buildInfoChip(IconData icon, String label, Color color) {
    return Container(
      ...
      decoration: BoxDecoration(color: color.withValues(alpha: 0.5), borderRadius: BorderRadius.circular(12)),
      child: Row(mainAxisSize: MainAxisSize.min, children: [
        Icon(icon, size: 12),
        const SizedBox(width: 4),
        Flexible(child: Text(label, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w500), overflow: TextOverflow.ellipsis)),
      ]),
    );
  }
    ...
    return Padding(
      ...
      child: Form(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          Text('Quick Edit', style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w800)),
          if (_error != null) ...[
            const SizedBox(height: 8),
          ...
          TextFormField(
            ...
            decoration: InputDecoration(labelText: 'Name', border: OutlineInputBorder(borderRadius: BorderRadius.circular(14)), filled: true),
          ...
          TextFormField(
            ...
            decoration: InputDecoration(labelText: 'Quantity', border: OutlineInputBorder(borderRadius: BorderRadius.circular(14)), filled: true),
            validator: (value) {
              if (value == null || value.trim().isEmpty) return 'Quantity is required';
              final parsed = int.tryParse(value.trim());
              if (parsed == null) return 'Must be a valid number';
              if (parsed < 0 || parsed > InventoryItem.maxQuantity) return 'Must be 0-${InventoryItem.maxQuantity}';
              return null;
            },
          ),
          const SizedBox(height: 12),
          TextFormField(
            ...
            maxLines: 2,
            decoration: InputDecoration(labelText: 'Note', border: OutlineInputBorder(borderRadius: BorderRadius.circular(14)), filled: true),
          ),
          const SizedBox(height: 16),
          Row(children: [
            Expanded(
