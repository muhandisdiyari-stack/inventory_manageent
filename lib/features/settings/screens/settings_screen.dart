...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Future<void> _saveSettings(InventoryProvider provider) async {
    _ensureQuantityField();
  ...
  void _addCustomField() {
    ...
    if (_standardFields.any((f) => f.toLowerCase() == lower)) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('"$field" is a standard field'),
    ...
    final isDuplicate = _customFields.any((f) => f.toLowerCase() == lower);
    if (isDuplicate) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            ...
            if (_hasChanges)
              Padding(
                ...
                child: TextButton.icon(
                  onPressed: _isSaving ? null : () => _saveSettings(provider),
                  icon: _isSaving
                      ? const SizedBox(
                          width: 16,
                          height: 16,
        ...
        child: Column(
          ...
          children: [
            ...
            ..._fieldConfigs.asMap().entries.map(
                  (entry) => _buildFieldTile(entry.value, entry.key),
                ),
          ],
        ),
      ),
              ...
              builder: (_, value, __) {
                return Row(
                  children: [
                    ...
                    const SizedBox(width: 8),
                    FilledButton(
                      onPressed: value.text.trim().isNotEmpty ? _addCustomField : null,
